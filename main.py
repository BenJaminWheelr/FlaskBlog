...
def register():
    ...
    if form.validate_on_submit():
        ...
        if already_exists:
            flash('An account under this email already exists. Try logging in.')
            return redirect(url_for('login'))
        name = form.name.data.title()
        unhashed_pass = form.password.data
        hashed_pass = generate_password_hash(unhashed_pass, "pbkdf2:sha256", 8)
        new_user = User(
            email = email,
